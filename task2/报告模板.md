...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] != 0)                                                                             //              ****
        {
            arr_small[count2++] = arr_big[i];	//将在arr_big[]中出现过的字母，放到arr_big中                            **** 
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //如果该节点是叶节点                                            **** 
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //遇到没有父节点和子节点的（没有出现的节点）就结束循环                               **** 
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //找到第一个权重小于min的节点，并将其值做为min的新值               **** 
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    {
    	if(huffman[t][3] == -1 && huffman[t][0] == -1)
    	{
    		break;
		}
		if(t !=ii[0] && huffman[t][3]>=huffman[ii[0]][3] && huffman[t][0] == -1 && huffman[t][3]<min1)
		{
			min1 = huffman[t][3];
			ii[1]=t;
		}
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //    表示该节点是根节点 ，没有父节点                                                                                  **** 
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        if(huffman[father][1] == current)	//判断当前结点的父结点左子树是否为当前结点                                                     /*自行填充代码*/  
       		{
...
（1）DEAEDDEDBCE![0d5c7448f8cac5b55c38af6d61d9fe2](./assets/0d5c7448f8cac5b55c38af6d61d9fe2-1682683798046-2.png)EECECDEEDDBEEECEEDEE



（2）EEBEACCCCAADACCBCCCCECCCEEECCCE

![04dae30e4222530ec957df817faeed1](./assets/04dae30e4222530ec957df817faeed1-1682683809877-5-1682683817738-7.png)
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？第1次必定是2个叶子组成二叉树,产生1新结点,也就是说做了一次计算，增加了一个节点，然后每次都是如此，会有n-1次运算，从而产生了 根节点 ，所以会有一共2n-1个节点
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？if(str[i] == (char)(0x41 + j))换成if(str[i] == (char)(0x61 + j))思路用一个if循环，该循环可以判断输入是不是大小写，如果是小写进入if(str[i]==(char)(0x41+j))如果是大写就进入另一个
3. 使用画图工具画出上述两个Huffman生成树的生成过程

![36dad12ec201e4a12ce4aff1148d0e0](./assets/36dad12ec201e4a12ce4aff1148d0e0-1682683864645-10.png)



![7ae44cc887fff1142e7d29e8421b1e8](./assets/7ae44cc887fff1142e7d29e8421b1e8-1682683875062-13.png)
